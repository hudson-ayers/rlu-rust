...
use rlu::{ConcurrentBTreeSet, ConcurrentSet, RluSet};
...
fn main() {
    let matches = App::new("RLU benchmark")
        .arg(
            Arg::with_name("settype")
                .short("s")
                .possible_values(&["rlu", "bt"])
                .default_value("rlu"),
        )
    ...
    let opts = BenchOpts {
        ...
        num_iters: matches
            ...
            .unwrap()
            .parse::<usize>()
            .expect("Failed to parse num_iters"),
        timeout: matches
            .value_of("timeout")
            .unwrap()
            .parse::<u128>()
